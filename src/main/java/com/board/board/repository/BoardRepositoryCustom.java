...
public class BoardRepositoryCustom {
	...
	public List<BoardListVo> getBoardList(Pageable pageable) {
		return queryFactory
			.select(new QBoardListVo(board.id, board.createdDate, board.title, board.writer, board.content, user.id,
				board.view, board.thumbnail, board.subcontent, board.likecnt, board.commentcnt, user.picture,
				board.hashTag, board.isfull))
			.from(board)
			.leftJoin(user)
